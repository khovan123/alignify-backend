...
public class ContentPostingService {
    ...
    public ResponseEntity<?> createContentPosting(ContentPosting contentPosting, HttpServletRequest request) {
        ...
        return ApiResponse.sendSuccess(201, "Content posting created successfully", contentPosting, request.getRequestURI());
    }
    ...
    if (contentPostingOpt.isPresent()) {
        ...
        if (!Helper.isOwner(contentPosting.getInfluencerID(), request) || !"1".equals(currentUserId)) {
    ...
    public ResponseEntity<?> updateContentPosting(String contentId, ContentPosting updatedContentPosting, HttpServletRequest request) {
        Optional<ContentPosting> contentPostingOpt = contentPostingRepo.findById(contentId);
        if (contentPostingOpt.isPresent()) {
            ContentPosting contentPosting = contentPostingOpt.get();
            ...
            return ApiResponse.sendSuccess(200, "Content posting updated successfully", contentPosting, request.getRequestURI());
        } else {
            return ApiResponse.sendError(404, "Content posting not found", request.getRequestURI());
        }
    }
    ...
    public ResponseEntity<?> incrementLike(String contentId, HttpServletRequest request) {
        ...
        if (contentPostingOpt.isPresent()) {
            ...
            contentPostingRepo.save(contentPosting);
            return ApiResponse.sendSuccess(200, "Like incremented", Map.of("like", contentPosting.getLike()), request.getRequestURI());
        } else {
            return ApiResponse.sendError(404, "Content posting not found", request.getRequestURI());
        }
    }
